...
```bash
# 1. 克隆
git clone https://github.com/lenkacos-dot/uninstall-scout
...
1. **扫描** → 找出所有卸载残留
2. **展示** → 列出每个软件名下的文件、大小、删除理由
3. **询问** → 让用户选择要删哪些（支持 `1,3-5,7` 或 `a` 全选）
4. **确认** → 再次询问「确认删除以上 N 个文件？(y/N)」
5. **执行** → 确认后才移到废纸篓
...
| 对比项 | Uninstall Scout | Tencent Lemon |
|--------|----------------|---------------|
